...
uses graph,crt;                           {Подключаем модули}
var
vga,vgahi:integer;                    {Переменные для графического режима}
n: word;                              {Количество звёзд}
x,y: integer;
...
InitGraph(vga, vgahi ,'c:\prg\bp\bgi');        {Подключаем видеорежим}
For n:=1 to 800 do
   begin
   a[n]:=Random(1280);                          {Разрешение экрана по Х}
   b[n]:=Random(1024);                          {Разрешение экрана по Y}
   end;
